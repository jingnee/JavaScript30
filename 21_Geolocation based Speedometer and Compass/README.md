...
`getCurrentPosition`은 현재위치를 1번만 보여주고

`watchPosition`은 현재위치를 계속 보여준다. 움직임에 따라서!



![image](https://user-images.githubusercontent.com/30755941/79050713-d3ca9980-7c66-11ea-9597-4b7eb921a322.png)

`timestamp` : 언제 발생했는지를 보여준다.

[UNIX timestamp 변환](https://www.epochconverter.com/) 이사이트에서 timestamp값을 입력하면 읽을 수 있는 날짜로 변화되어서 보여준다.

![image](https://user-images.githubusercontent.com/30755941/79050725-ed6be100-7c66-11ea-9d8a-3733c97c89f0.png)
...
navigator.geolocation.watchPosition((data) => {
      ...
      arrow.style.transform = `rotate(${data.coords.heading}20deg)`;
    }, (err) => {
      console.error(err);
...
speed와 arrow방향을 `data.coords`에서 가져와서 설정해준다.

에러가 나타나면 알림창을 뜨게 해준다.
